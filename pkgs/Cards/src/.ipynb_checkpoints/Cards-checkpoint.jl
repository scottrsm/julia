...
"""
   Comparison function used for slot, lt in a sort function.
...
function pokerHandCmp(hr1::PHrep, hr2::PHrep)
    ...
    if handType in [ThreeOfAKind, FullHouse]
        rp1, sp1 = get_hand_piece(hr1, 1)
        rp1, sp2 = get_handpiece(hr2, 1)
    ...
    if handType in [Singles, Flush, Straight, StraightFlush]  # Compare the highest cards by suit.
        return(s1 < s2)
    elseif handType == Pair  # Compare the next highest card by card rank, if the same, compare by suit.
        if r1 < r2
            return(true)
        elseif r1 > r2
            return(false)
    ...
    elseif handType == TwoPair # Compare the next highest pair by card rank, if the same, compare by the next single card: first by rank and then by suit.
        rr1, ss1 = get_card_rank_and_top_suit(hr1, 2)
        rr2, ss2 = get_card_rank_and_top_suit(hr2, 2)
        if rr1 < rr2
            return(true)
...
function playPoker2!(d::Deck)
    ...
    if islessFlag
        ...
        println("Player 1 wins!")pokerTrad
    end
